module.exports = (navigationViewToImport) =>
 {
    ...
    let handleAllJobs = require("../helpers/actionIcons.js")(createMenuActionIcon,"All","srcImg","low",navigationViewToImport);
            ...
            xhrRetrieveJobs.addEventListener("loadstart", () =>
             {
               ...
               activityIndicator = new tabris.ActivityIndicator({width:40,height:40,centerX:0,centerY:0}).appendTo(scrollView);  
             });
               ...
               if(response.Message === "Pas de resultat")
                {
                    ...
                    let imageNoResult = new tabris.ImageView({
                        centerX:0, centerY:0,
                        image:{src: "img/empty_search.png"},
                        width:48, height:48,
                        id:"imageSearch"
                      }).appendTo(scrollView);
                   
                    let textNoConnection = new tabris.TextView({
                        top:["#imageSearch", 10], centerX:0,
               ...
               else
                {
                    ...
                    for(let i = 0; i<j; i++)
                      {
                        objectData =  {titre:response.titre[i],categorie:response.categorie[i], date_mise_en_ligne:response.date_mise_en_ligne[i], lieu_emploi:response.lieu_emploi[i], nom_emploi:response.nom_emploi[i], description:response.description[i], photo_emploi:response.photo_emploi[i]};
                        itemJobs.push(objectData);
                      }
                    let jobsCollectionView = new tabris.CollectionView({
                        right:10, bottom:0, top:0,left:0,
                        ...
                        createCell: () =>{
                          let cell = new tabris.Composite({
                           ...
                           new tabris.Composite({left: 75, bottom: 0, right: 10, height: 1,background: '#eeeeee'}).appendTo(cell);
            
                           let imageViewCell = new tabris.ImageView({
                               left:10,top:10,width:50,height:50,cornerRadius:25,
                               scaleMode:"fill",
                               id:"imageViewCell"
                             }).appendTo(cell);
         
                           let firstTextViewCell = new tabris.TextView({
                               left: 75, top:15,right:15,
                               maxLines:1,
